...
typedef struct PCB
{
    ...
    int cpuBurst;
    int waitingTime;
    int priority; //lower the value, lower the priority
    struct PCB *next;
    struct PCB *prev;
}PCB;
...
PCB *head = NULL;
PCB *tail = NULL;
int waitingTime = 0, numProcess = 0;

void addProcess(int PID, int CB, int priority)
{
    ...
    {
        PCB *temp1 = head;
        while (temp1 != NULL && temp1 -> priority <= priority)
        {
            temp1 = temp1 -> next;
        }
...
int main()
{
    ...
    {
        int PID, CB, priority;
        printf("\nEnter process ID, CPU Burst Time and priority: ");
        scanf("%d %d %d", &PID, &CB, &priority);                
        addProcess(PID, CB, priority);
    }
    waitingTimeCalc();
